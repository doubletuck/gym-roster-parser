...
public class WesternMichiganScraper extends AbstractScraper {

    public final static Logger logger = LoggerFactory.getLogger(WesternMichiganScraper.class);

    public WesternMichiganScraper(Integer year) {
        super(year);
    }

    public College getCollege() {
        return College.WESTERNMICHIGAN;
    }

    String buildRosterUrl() {
        return String.format("%s/%d?view=2",
                "https://wmubroncos.com/sports/womens-gymnastics/roster",
                this.year);
    }
    ...
    Athlete parseAthleteRow(Element tableRowElement) {
        ...
        if (this.year <= 2021 && this.year > 2016) {
            eventIndex = -1;
