...
public class TowsonScraper extends AbstractScraper {

    public final static Logger logger = LoggerFactory.getLogger(TowsonScraper.class);

    public TowsonScraper(Integer year) {
        super(year);
    }

    public College getCollege() {
        return College.TOWSON;
    }

    String buildRosterUrl() {
        return this.year > 2015 ?
                String.format("%s/%d?view=2",
                        "https://towsontigers.com/sports/womens-gymnastics/roster",
                        this.year) :
    ...
    Athlete parseAthleteRow(Element tableRowElement) {
        ...
        if (this.year == 2024 || (this.year <= 2014)) {
            nameIndex = 1;
            eventIndex = 2;
            academicYearIndex = 4;
            hometownIndex = 5;
